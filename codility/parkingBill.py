...
def solution(E, L):
    # Assuming element on first index are hours and second index minutes.
    try: 
        e_time = [int(ele) for ele in E.split(':')]
        l_time = [int(ele) for ele in L.split(':')]
        time_stamps = []
        fee = 0
        # 
        for t in range(e_time[0], l_time[0]+1):
